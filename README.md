...
Используйте доверенный архив и отдельное SSH-подключение, не зависящее от восстанавливаемого VPN. Для нового сервера сначала установите WireGuard и необходимые инструменты.

1. Посмотрите содержимое и распакуйте архив в отдельный закрытый каталог, а не сразу в `/`:

   ```bash
   sudo tar -tzf /var/backups/wireguard/ИМЯ-АРХИВА.tar.gz
   sudo install -d -m 700 /root/wg-restore
   sudo tar -xzf /var/backups/wireguard/ИМЯ-АРХИВА.tar.gz -C /root/wg-restore
   ```

   Используйте пустой каталог для каждого восстановления. Не публикуйте содержимое файлов с ключами.

2. Сохраните текущую конфигурацию отдельным бэкапом. Остановите нужный интерфейс: `sudo systemctl stop wg-quick@wg0` (Alpine: `sudo rc-service wg-quick.wg0 stop`). Замените `wg0` своим именем. Установка в режиме `auto` при остановке выполнит текущие firewall-хуки.
3. Сравните распакованные файлы с действующими. Перенесите нужные файлы из `/root/wg-restore/etc/wireguard/` в `/etc/wireguard/`, сохранив владельца root и права `600` для секретных файлов, `700` для каталога. На свежем сервере создайте каталог с этими правами. Файлы клиентов лежат в подкаталогах `root/`, `home/` или по указанным при бэкапе путям; восстановите их в подходящие домашние каталоги, проверив владельцев и права. На другом сервере числовые UID/GID пользователей могут отличаться.
4. При необходимости восстановите `/etc/sysctl.d/wg.conf` и примените его: `sudo sysctl -p /etc/sysctl.d/wg.conf`. Восстановите собственные правила nftables отдельно. Проверьте имя внешнего интерфейса, порт, подсети, наличие IPv6 и старые `PostUp`/`PostDown`. Если внешний адрес сервера изменился, обновите `SERVER_PUB_IP` в `params` и `Endpoint` в уже выданных клиентских конфигурациях.
5. Запустите интерфейс: `sudo systemctl start wg-quick@wg0` (Alpine: `sudo rc-service wg-quick.wg0 start`). На свежем сервере также настройте автозапуск службы; для Alpine нужен сервис `wg-quick.wg0`, как при обычной установке.
6. Проверьте `sudo wg show`, handshake с клиентом, доступ в интернет и DNS, отдельно IPv4/IPv6 при их использовании. Восстановление старого архива может вернуть ранее отозванных клиентов — проверьте список peers перед запуском.

Сначала проверьте процедуру на тестовом сервере. Сохранённые ключи позволяют существующим клиентам подключаться без перевыпуска конфигураций, если адрес сервера и остальные параметры остаются прежними.
